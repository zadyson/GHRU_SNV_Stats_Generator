...
# Get list of files
files <- list.files(path="~/Dropbox/2021_TGC_WG2/hets_all/bcfs_manual_screen", pattern="*.vcf", full.names=TRUE, recursive=FALSE)

# Iterate through files and plot each
for (file in 1:length(files)){
  
  # Import and reformat vcf file into table
  vcf <- read.vcfR(files[file],
                   verbose = TRUE )
  vcf2 <- tbl_df(vcf@fix)
  ...
  # plot het SNVs before exclusion
  plot_points <- vcf3 %>%
  ...
  # plot het qual SNVs after exclusion
